...
def quiz(y):
    ...
    Button(frame1, text="Removing contact with heat source immediately",font=("Arial",10), bg="black", command=a1_right).grid(row=3, column=1)
    Button(frame1, text="Removing cloth/jewelry even if stuck to the skin",font=("Arial",10), bg="black", command=a1_wrong).grid(row=3, column=2)
    Button(frame1, text="Removing cloth/jewelry near burnt area",font=("Arial",10), bg="black",command=a1_right).grid(row=3, column=3)

    Label(frame2, text="Treatment for burns for all ages includes", font=("Arial", 20)).grid(row=2, column=2)
    Button(frame2, text="Paracetamol",font=("Arial",10), bg="black",command=a2_right).grid(row=3, column=1)
    Button(frame2, text="Ibuprofen",font=("Arial",10), bg="black",command=a2_right).grid(row=3, column=2)
    Button(frame2, text="Aspirin",font=("Arial",10), bg="black",command=a2_wrong).grid(row=3, column=3)

    Label(frame3, text="If the subject had an electrical burn", font=("Arial", 20)).grid(row=2, column=2)
    Button(frame3, text="Call emergency medical help",font=("Arial",10), bg="black",command=a3_right).grid(row=3, column=1)
    Button(frame3, text="Wait to see if they are feeling pain",font=("Arial",10), bg="black",command=a3_wrong).grid(row=3, column=2)
    Button(frame3, text="Help the subject sit down no matter how high the voltage source",font=("Arial",10), bg="black",command=a3_wrong).grid(row=3, column=3)

    Label(frame4, text="If the subject had a chemical burn...", font=("Arial", 20)).grid(row=2, column=2)
    Button(frame4, text="Dry any liquid by rubbing with a napkin",font=("Arial",10), bg="black",command=a4_wrong).grid(row=3, column=1)
    Button(frame4, text="Brush off any dry chemical",font=("Arial",10), bg="black",command=a4_right).grid(row=3, column=2)
    Button(frame4, text="Use running water to remove the chemical without rubbing it",font=("Arial",10), bg="black",command=a4_right).grid(row=3, column=3)

    Label(frame5, text="Immediately call an ambulance if the subject of the burn...", font=("Arial", 20)).grid(row=2, column=2)
    Button(frame5, text="Is going into shock (sweating, shallow breathing, cold and clammy skin...)",font=("Arial",10), bg="black",command=a5_right).grid(row=3, column=1)
    Button(frame5, text="If over 60 or under 5",font=("Arial",10), bg="black",command=a5_right).grid(row=3, column=2)
    Button(frame5, text="Has a medical condition/weakened immune system",font=("Arial",10), bg="black",command=a5_right).grid(row=3, column=3)
